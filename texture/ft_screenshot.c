...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/04 11:03:37 by ayajrhou          #+#    #+#             */
/*   Updated: 2021/02/04 11:03:40 by ayajrhou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

void				direction_global(void)
{
	g_direction.ea = 0;
	g_direction.no = 0;
	g_direction.su = 0;
...
void				ft_header(void)
{
	...
	g_screenmap.width_in_byte = ((g_screenmap.g_width *
		g_screenmap.bit_count + 31) / 32) * 4;
...
void				create_image(void)
{
	...
	while (row > 0)
	{
		...
		{
			colors = get_color(g_data[(g_screenmap.g_height - row) *
				g_screenmap.g_width + col]);
			g_screenmap.img[row * g_screenmap.width_in_byte +
				col * 3 + 2] = colors[0];
			g_screenmap.img[row * g_screenmap.width_in_byte +
				col * 3 + 1] = colors[1];
			g_screenmap.img[row * g_screenmap.width_in_byte +
...
{
	g_screenmap.fd = open("./screenshot.bmp",
